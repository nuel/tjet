...
The tjet server does not include a client http server and acts only as a backend service. Host the page you want add tjet to using whatever method you prefer, then attach `tjet.css` and `tjet.js` to your page. The socket.io client library [(cdnjs link)](https://cdnjs.com/libraries/socket.io) should also be included. Then add a div somewhere with the id `#tjet`. This div can be styled however you want.
